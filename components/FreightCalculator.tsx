...
import { Ship, Anchor, ArrowRight, ArrowLeft, DollarSign, Calculator } from 'lucide-react';
...
const FreightCalculator: React.FC = () => {
  ...
  const handleCalculate = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // Simulate random pricing logic for demo purposes
      ...
      <div className="p-8">
        <form onSubmit={handleCalculate} className="space-y-6">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className={`w-full py-4 bg-secondary text-white font-bold rounded-lg hover:bg-secondary-hover transition-all shadow-md flex items-center justify-center gap-2 ${loading ? 'opacity-75 cursor-not-allowed' : ''}`}
          >
          ...
          <div className="mt-8 animate-fade-in-up">
            <div className="border-t border-slate-100 pt-6">
              ...
              <div className="bg-green-50 border border-green-100 rounded-xl p-6 text-center relative overflow-hidden">
                ...
                <a 
                  href="#contact" 
                  className="mt-4 inline-block text-primary font-bold hover:underline text-sm"
                >
                  {t.quote.contactUs}
                </a>
              </div>
